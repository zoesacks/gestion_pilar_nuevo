...
async function obtenerDocumentos() {
    ...
    const response = await fetch(`${url}/api/seguimiento-de-expedientes/documentos`)
...
function mostrarDocumentos(documentos) {
    ...
    filtrados.forEach(documento => {
        ...
        if (pendienteDeAceptar) {
            cantDocumentosPendientes++
        } else if (enviadoPorUsuario) {
            cantDocumentosEnTransito++
        } 
        ...
        ejercicioCell.classList.add('d-none', 'd-sm-table-cell');
        tipoDescripcionCell.classList.add('d-none', 'd-sm-table-cell');

        ejercicioCell.classList.add('d-none', 'd-md-table-cell');
        tipoDescripcionCell.classList.add('d-none', 'd-md-table-cell');
        ...
        if (esPropio) {
            ...
            <button type="button" class="btn btn-primary transferir-documento-boton" data-bs-toggle="modal" data-bs-target="#transferirDocumentoModal">
            <i class="fa fa-paper-plane" aria-hidden="true"></i>
            </button>`
        }

        accionesCell.innerHTML += `
           <button type="button" class="btn btn-primary ver-documento-boton" data-bs-toggle="modal" data-bs-target="#verDocumentoModal">
...
async function obtenerDocumento(id) {
    const response = await fetch(`${url}/api/seguimiento-de-expedientes/documentos/${id}`)
    ...
    <div class="card border-0">
        <div class="card-body">
            <div class="section">
                        ...
                        <p><b class="text-primary text-uppercase">Observaciones: </b>${documento.observacion ? documento.observacion : 'No hay observaciones'}
...
async function obtenerUsuarios() {
    const response = await fetch(`${url}/api/seguimiento-de-expedientes/usuarios`)
